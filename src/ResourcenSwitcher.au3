...
;		- Fensterposition auf Bildschirm zur�ckholen
...
;Schreibrechte pr�fen
Local $tempFileName=_TempFile(@ScriptDir,"~")
Local $tempFile=FileOpen($tempFileName,2)
If $tempFile=-1 Then
	MsgBox(48, $ToolName, "Dieses Programm braucht Schreibrechte in seinem eigenen Programmverzeichnis."&@CRLF&"Bitte starte es als Administrator oder gew�hre die Schreibrechte manuell."&@CRLF&"Das Programm wird beendet.")
	Exit
EndIf
FileClose($tempFile)
...
If ($EEPVersionsCount < 1) Then
	If 6 = MsgBox(4+32,$ToolName, "Das Programm kennt noch keine EEP-Versionen, f�r die Resourcenordner gewechselt werden k�nnten."&@CRLF&"Soll im Internet danach gesucht werden?") Then
		Update()
	EndIf
...
For $i = 1 To $EEPVersionsCount
	;Pr�fen, ob die EEP-Version (in der Registry) existiert
...
;~  		MsgBox(0,$ToolName,IniRead($IniFileName,$IniSectionsVersions&$i,"Name","???")&" gibt es nicht")
	Else
		...
		;Pr�fen, ob "ResBase" in der Registry steht
...
Global $EEPVersionAkt = 1 ;wird sp�ter per SetVersion neu gesetzt
...
;Unterschied zwischen Fenstergr��e und Clientsize rausfinden
...
;~ ;MsgBox(0,"Fenstergr��e","Angelegt mit 285x175px"&@CRLF&"Aktueller Clientbereich: "&$SizeClient[0]&"x"&$SizeClient[1]&"px"&@CRLF&"Aktuelle Fenstergr��e: "&$SizeBorder[2]&"x"&$SizeBorder[3]&"px")
...
;ImageList f�r die EEP-Icons
Global $ImageList_EEPIcons = _GUIImageList_Create(16, 16, 4, 1)

;Tabs zum Ausw�hlen der EEP-Version
...
;~ 	;MsgBox(0,"Gesamt-Zahl an Icons",_GUIImageList_GetImageCount($ImageList_EEPIcons))
	...
	;Liste zum Ausw�hlen des Resourcenordners
	...
	;Button Hinzuf�gen
	_ArrayAdd($Buttons_Add, GUICtrlCreateButton("&Hinzuf�gen", 220, 0, 25, 25, $BS_ICON))
	...
	GUICtrlSetTip(-1, "Neuen Resourcen-Ordner zur Liste hinzuf�gen")
	GUICtrlSetImage(-1, @ScriptFullPath, -5, 0)
;~ 	;_GUICtrlButton_SetImageList($Buttons_Add[$i],$ImageList,4)

	;Button �ndern
	_ArrayAdd($Buttons_Edit, GUICtrlCreateButton("�&ndern", 0, 30, 25, 25, $BS_ICON))
	GUICtrlSetResizing(-1, $GUI_DOCKRIGHT + $GUI_DOCKTOP + $GUI_DOCKHEIGHT + $GUI_DOCKWIDTH)
	GUICtrlSetOnEvent($Buttons_Edit[$i], "ShowGUIResourcenFolderEdit")
	GUICtrlSetTip(-1, "Ausgew�hlten Resourcen-Ordner bearbeiten")
	GUICtrlSetImage(-1, @ScriptFullPath, -6, 0)

	;Button L�schen
	_ArrayAdd($Buttons_Delete, GUICtrlCreateButton("&L�schen", 0, 30, 25, 25, $BS_ICON))
	GUICtrlSetResizing(-1, $GUI_DOCKRIGHT + $GUI_DOCKTOP + $GUI_DOCKHEIGHT + $GUI_DOCKWIDTH)
	GUICtrlSetOnEvent($Buttons_Delete[$i], "DeleteResourcenFolder")
	GUICtrlSetTip(-1, "Ausgew�hlten Resourcen-Ordner aus der Liste entfernen")
	GUICtrlSetImage(-1, @ScriptFullPath, -7, 0)

Next

GUICtrlCreateTabItem("") ;Erstellung der Tabs abschlie�en
...
GUICtrlSetState($TabItems_EEPVersions[$EEPVersionAkt],$GUI_SHOW) ;und den entsprechenden Tab vorw�hlen
GUISetState(@SW_SHOW, $GUI)
ResizeWindow()
GUIRegisterMsg($WM_GETMINMAXINFO, "WM_NOTIFY") ; Minimale Gr��e festlegen
...
;~ 	If $Index = "" Then
;~ 		MsgBox(48, $ToolName, "Du musst einen Resourcen-Ordner zum Wechseln ausw�hlen", Default, $GUI)
;~ 		Return
;~ 	EndIf
	...
	If RegRead($EEPRegPath, "ResBase") <> $path Then
		...
		If @error Then
			MsgBox(48, $ToolName, "Bitte starte dieses Programm mit Schreibrechten in der Registry, z.B. als Administrator." & @CRLF & _
					"Die Zugriffsrechte k�nnen sich auf folgenden Pfad/Schl�ssel beschr�nken:" & @CRLF & _
					$EEPRegPath)
			DisplayResourcenFolders($EEPVersionAkt)
			Return
		EndIf
...
Func SwitchLinkTo($Index)
	...
	If FileExists($homeRes) And Not DirIsLink(RegRead($EEPRegPath, "Directory"), "Resourcen") Then
		If MsgBox(20, $ToolName, $homeRes &@CRLF& "ist noch ein vollwertiger Resourcen-Ordner, sodass an " & @CRLF & _
		  "seiner Stelle keine Verkn�pfung erstellt werden kann."&@CRLF&@CRLF& _
...
;~ 			MsgBox(0,_GUICtrlListView_GetItemText($ListViews_ResourcenFolders[$EEPVersionAkt],$EntryToEdit-1,2),$EntryToEdit-1)
		...
		Return
	EndIf
	If Not FileExists($path) Then ;Pr�fen, ob der Zielordner existiert
		MsgBox(48, $ToolName, "Der Ordner"&@CRLF&$path&@CRLF&"existiert nicht und kann daher nicht als Linkziel festgelegt werden.")
		Return
	ElseIf StringInStr(FileGetAttrib($path), "D") = 0 Then ;Pr�fen, ob der Zielordner auch wirklich ein Ordner ist.
...
;~ 		_GUICtrlListView_SetItemImage($ListView_ResourcenFolders, $Index, 2, 1)
		...
		If LinkGetRealDir($homeRes) <> $path Then ;Falls der Link von LinkGetRealDir nicht richtig aufgel�st werden kann, den Pfad in eine versteckte Textdatei schreiben
		...
		SwitchRegistryTo(-1)
	Else
		MsgBox(48, $ToolName, "Um die Verkn�pfung zu erstellen, sind Schreibrechte im EEP-Verzeichnis n�tig."&@CRLF&"Bitte starte dieses Programm als Administrator oder gew�hre die Schreibrechte manuell.")
	EndIf
...
Func RenameResourcenFolder()
	...
	If FileExists($path) Then
		MsgBox(48,$ToolName,"Der gew�hlte Ordner existiert bereits.")
		Return
	EndIf
...
Func Update()
	...
	If $UpdateVersionsCount<=0 Then
		$UpdateMessage="Die heruntergeladene Datei enth�lt keine Informationen zu EEP-Versionen"
		;MsgBox(48,$ToolName,"Beim Update ist ein Fehler aufgetreten."&@CRLF&"Die heruntergeladene Datei enth�lt keine Informationen zu EEP-Versionen")
	Else
		...
		For $i=1 To $UpdateVersionsCount
			...
			_ArrayDelete($Section,0) ; IniReadSection gibt noch die Anzahl zur�ck, die braucht IniWriteSection nicht
		...
		Else
			$UpdateMessage="Es wurden "&$NewVersions[0]&" neue EEP-Versionen zur Liste hinzugef�gt:"&@CRLF&_ArrayToString($NewVersions,@CRLF,1)
			;MsgBox(64,$ToolName,"Es wurden "&$NewVersions[0]&" neue EEP-Versionen zur Liste hinzugef�gt:"&@CRLF&_ArrayToString($NewVersions,@CRLF,1))

			;Neu starten, damit die neuen Eintr�ge erkannt werden
			$Restart=True
			;Close()
		EndIf
	EndIf
	...
	If _StringCompareVersions($CurVersion,$NewVersion)<0 Then
		...
		If 6 = MsgBox(4+64+256,$ToolName,$UpdateMessage&@CRLF&@CRLF& _
				...
				"Soll die Seite jetzt ge�ffnet werden?") Then
			ShellExecute($url)
		EndIf
	Else
...
Func ShowGUIResourcenFolderAdd()
	WinSetTitle($ResourcenFolder_GUI, "", "Resourcen-Ordner hinzuf�gen")
...
Func ShowGUIResourcenFolderEdit()
	...
	If $SelectedIndex = "" Then
		MsgBox(48, $ToolName, "Du musst einen Eintrag zum �ndern ausw�hlen", Default, $GUI)
		Return
	EndIf
...
Func DeleteResourcenFolder()
	Local $SelectedIndex = _GUICtrlListView_GetSelectedIndices($ListViews_ResourcenFolders[$EEPVersionAkt], False)
	If $SelectedIndex = "" Then
		MsgBox(48, $ToolName, "Du musst einen Eintrag zum L�schen ausw�hlen", Default, $GUI)
		Return
	EndIf
	Local $ItemTextArray = _GUICtrlListView_GetItemTextArray($ListViews_ResourcenFolders[$EEPVersionAkt])
	If $ItemTextArray[4] = RegRead($EEPVersions_RegPath[$EEPVersionAkt], "ResBase") Then
		MsgBox(48, $ToolName, "Du kannst nicht den aktuellen Resourcen-Ordner l�schen", Default, $GUI)
		Return
	EndIf
	If MsgBox(292, $ToolName, "Soll dieser Resourcen-Ordner wirklich aus der Liste gel�scht werden?" & @CRLF & $ItemTextArray[3] & @TAB & $ItemTextArray[4], Default, $GUI) = 6 Then
		Local $Paths = $ResourcenFolder_Paths[$EEPVersionAkt]
		Local $Descriptions = $ResourcenFolder_Descriptions[$EEPVersionAkt]
...
Func BrowsePath()
	Local $Folder = FileSelectFolder("Resourcen-Ordner w�hlen", "", 4, RegRead($EEPRegPath, "Directory"), $ResourcenFolder_GUI)
	If $Folder Then
...
;~ 	MsgBox(0,$ActResFolderIndex,$ActLinkIndex)
	For $i = 1 To UBound($ResourcenFolder_Descriptions[$v]) - 1
		...
		If StringInStr(FileGetAttrib($Paths[$i]), "D") = 0 Then
			GUICtrlSetColor(-1, 0xCCCCCC)		;Nicht vorhandene Ordner grau f�rben
		EndIf
...
;~ 	Local $Height = 0;_GUICtrlListView_GetItemCount($ListView_ResourcenFolders)*17+150
	...
	If ($Left<0 Or $Left>@DesktopWidth-$Width Or $Top<0 Or $Top>@DesktopHeight-$Height) And _
		MsgBox(36,$ToolName,"Das Fenster scheint au�erhalb des Bildschirms zu liegen." & @CRLF & "Soll es zur�ckgeholt werden?")=6 Then
...
;~ 					;MsgBox(0,$ToolName,"Mit den Tabs ist was los: "&$ID&$iCode)
	...
	EndSwitch
	Switch $uMsg
		Case $WM_GETMINMAXINFO ; Die Fenstergr��e abfragen, minimale Gr��e f�rs verkleinern setzen
